...
$(function () {
  for (let i = 1; i <= 3; i++) {
    $(`#tmp-${i}-sm`).text($(`#tmp-${i}`).text());
  }
  for (let i = 1; i <= 6; i++) {
    $(`#feat-${i}-sm`).text($(`#feat-${i}`).text());
  }
});
